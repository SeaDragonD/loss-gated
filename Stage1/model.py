...
class model(nn.Module):
    def __init__(self, lr, lr_decay, **kwargs):
        ...
        self.Scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer=self.OptimNet, milestones=[40, 60], gamma=lr_decay)
    ...
    def train_network(self, loader, epoch):
        ...
        for counter, data in enumerate(loader, start = 1):     
            ...
            sys.stdout.write("[%2d] Lr: %5f, %.2f%% (est %.1f mins) cLoss %f sLoss %f Loss %f EER/TAcc %2.3f%% \r"%(epoch, lr, 100 * (counter / loader.__len__()), time_used * loader.__len__() / counter / 60, closs, sloss, loss/counter, top1/counter))
            sys.stdout.flush()
        sys.stdout.write("\n")
    ...
    def evaluate_network(self, loader):
        ...
        with torch.no_grad():
            for counter, data in enumerate(loader, start=1):
                ...
                feat = []
                for inp in data:
                    feat.append(self.Network.forward(
                        torch.FloatTensor(inp).cuda()))  # Feed the segments to get the speaker embeddings
                feat = torch.stack(feat, dim=1).squeeze()

                # Train model

                in_AAT = torch.cat(
                    (torch.cat((feat[:, 0, :], feat[:, 1, :]), 1), torch.cat((feat[:, 0, :], feat[:, 2, :]), 1)), 0)
                out_AAT = self.AATNet(self.Reverse(in_AAT))
                closs = criterion(out_AAT, AAT_labels)  # AAT loss
                sloss, prec1 = self.Loss.forward(feat[:, [0, 2], :])  # speaker loss
                nloss = sloss + closs * 3  # Total loss
                ...
                time_used = time.time() - tstart  # Time for this epoch
                sys.stdout.write("[TEST] Lr: %5f, %.2f%% (est %.1f mins) cLoss %f sLoss %f Loss %f EER/TAcc %2.3f%% \r" % (
                 lr, 100 * (counter / loader.__len__()), time_used * loader.__len__() / counter / 60, closs, sloss,
                loss / counter, top1 / counter))
                sys.stdout.flush()
            sys.stdout.write("\n")
