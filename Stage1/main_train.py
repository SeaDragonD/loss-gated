...
parser.add_argument('--initial_model',     type=str,   default="exp/exp4/model/model000000035.model",           help='Initial model path')
...
if 'new' in args.test_list.split('/')[-1]:
    ...
    val_dataset = dataLoader_v2.get_testloader(args)
else:
    test_dataset = get_testloader(args)
    args.test_path = args.val_path
    val_dataset = get_testloader(args)
...
if args.eval == True: # Do evaluation only
    ...
    print('[TEST] avg_closs %2.4f, Acc %2.3f\n' % (avg_closs, Acc))
    avg_closs, Acc = Trainer.evaluate_network(loader=val_dataset)
    print('[VAL] avg_closs %2.4f, Acc %2.3f\n' % (avg_closs, Acc))
    quit()
...
while it < args.max_epoch:
    ...
    if it % args.test_interval == 0:
        ...
        print(time.strftime("%Y-%m-%d %H:%M:%S"),"[TEST] LR %f, Acc %2.2f, LOSS %f, avg_closs %2.4f, Acc %2.3f" % (lr, traineer, loss, avg_closs, Acc))
        scorefile.write("[TEST] Epoch %d, LR %f, Acc %2.2f, LOSS %f, avg_closs %2.4f, Acc %2.3f\n"%(it, lr, traineer, loss, avg_closs, Acc))
        scorefile.flush()

        avg_closs, Acc = Trainer.evaluate_network(loader=val_dataset)
        print(time.strftime("%Y-%m-%d %H:%M:%S"),
              "[VAL] LR %f, Acc %2.2f, LOSS %f, avg_closs %2.4f, Acc %2.3f" % (lr, traineer, loss, avg_closs, Acc))
        scorefile.write("[VAL] Epoch %d, LR %f, Acc %2.2f, LOSS %f, avg_closs %2.4f, Acc %2.3f\n" % (
        it, lr, traineer, loss, avg_closs, Acc))
        scorefile.flush()
